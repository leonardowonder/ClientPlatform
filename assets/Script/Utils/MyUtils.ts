    ...
    public reqConfig() {
        ...
        xhr.onload = function () {
            if (xhr.readyState == 4 && (xhr.status >= 200 && xhr.status < 400)) {
                ...
                if (json) {
                    //判断是否是苹果审核版本
                    if (cc.sys.isNative && this.GetPlatForm() == PlatForm.CC_PLATFORM_IOS) {
                        this.m_bIsCheck = json.version < this.getLocalVerfion() && json.isCheck == 0;
                    }
                    if (cc.sys.isNative && json.version > this.getLocalVerfion()) {
                        cc.find("persistRootNode").getComponent('persistRootNode').showMessageBox("发现新版本!", function () {
                            this.OpenUrl(json.durl);
                        }.bind(this));
                    } else if (json.isShowAlert == 1) {
                        cc.find("persistRootNode").getComponent('persistRootNode').showMessageBox(json.alert, function () {
                        }.bind(this));
                    }
                }
            }
        ...
        xhr.open("GET", "http://116.62.172.157/jl13d.php", true);
        xhr.send();
    }

    public isCheck() {
        return this.m_bIsCheck;// && cc.sys.isNative && this.GetPlatForm() == PlatForm.CC_PLATFORM_IOS;
    }
